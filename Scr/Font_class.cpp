...
bool FontShaderClass::InitializeShader( ID3D11Device* device, HWND hwnd, LPCWCHAR vsFilename, LPCWCHAR psFilename )
{
	...
	result = D3DCompileFromFile( psFilename, NULL, NULL, "TPShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,	&pixelShaderBuffer, &errorMessage );
	if( FAILED( result ) )
	{
		// If the shader failed to compile it should have writen something to the error message.
		if( errorMessage )
...
bool FontShaderClass::SetShaderParameters( ID3D11DeviceContext* deviceContext, XMFLOAT4X4 _worldMatrix, XMFLOAT4X4 view,
	XMFLOAT4X4 proj, ID3D11ShaderResourceView* texture, XMFLOAT4 pixelColor )
{
	UNREFERENCED_PARAMETER( _worldMatrix );
